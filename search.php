		...
		<!-- container -->
			...
			<div class="search-wrap">
				...
				if (getOption('search_no_albums')) {		//test of images search
					if ($numimages > 0) {
						echo '<ul class="nav search-item"><li><h4>'; printf(gettext('Images (%s)'), $numimages); echo '</h4></li></ul>';
					}
				} else {
					if (getOption('search_no_images')) {	//test of albums search
						if ($numalbums > 0) {
							echo '<ul class="nav search-item"><li><h4>'; printf(gettext('Albums (%s)'), $numalbums); echo '</h4></li></ul>';
						}
					} else {
						if ($total1 > 0) {					//test of albums and images search
							echo '<ul class="nav search-item"><li><h4>'; printf(gettext('Albums (%1$s) &amp; Images (%2$s)'), $numalbums, $numimages); echo '</h4></li></ul>';
						}
					}
				}
					...
					if ($numpages > 0) { ?>
						...
						<?php while (next_page()) { ?>
							<div class="news-truncate clearfix">
								...
								<div class="search-content clearfix">
									<?php echo shortenContent(strip_tags(getPageContent()), 120, getOption("zenpage_textshorten_indicator")); ?>
								</div>
							</div>
						<?php
						}
